...
def plot_confusion_matrix(y_true, y_pred, classes, name,
    ...
    with open(f'confusion_{name}.txt', 'w') as f:
        for i, el in enumerate(cm):
            classname = str(classes[i][0]).replace('-', '_').replace(' ', '_')
            f.write(f'{classname},')
            for np_entry in el:
                f.write(f'{np_entry},')
            f.write('\n')
...
def _optimizer(config, flo_model, lambdas=[], debug=False, mode='adamw'):
    if not debug:
        flo_model = flo_model.module
...
def main():
    ...
    train_loader = DataLoader( edu, batch_size=cfg.batch_size, num_workers=cfg.workers, shuffle=True, pin_memory=False, drop_last=True,  collate_fn=collate_fn)
